...
def CYGNSS_average_plot(folder_name, sigma, step_size_lon, step_size_lat, smooth):
    ...
    if lat_edges.size == 0 or lat_edges[-1] < max_lat:
        lat_edges = np.append(lat_edges, max_lat)

    lon_edges = np.arange(min_lon, max_lon + step_size_lon, step_size_lon)
    lon_edges = np.arange(min_lon, max_lon, step_size_lon)
    if lon_edges.size == 0 or lon_edges[-1] < max_lon:
    ...
    # Title
    ax.set_title("CYGNSS SR - Lake Barlee - Jan & Feb 2020", fontsize=32, pad=30)
